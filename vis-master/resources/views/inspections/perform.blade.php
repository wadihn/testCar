...
<form method="POST" action="{{ route('inspections.submit', $inspection) }}" id="inspection-form" data-total-steps="{{ $totalSections }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="wizard-panel {{ $sIdx === 0 ? 'active' : '' }}" id="panel-{{ $sIdx }}">
        ...
        <div class="wizard-panel-body">
            ...
            <div class="wizard-question {{ $question->is_critical ? 'critical' : '' }}">
                <div class="wq-header">
                    ...
                    @if($question->max_score > 0 && !in_array(is_object($question->type) ? $question->type->value : $question->type, ['text', 'photo']))
                        ...
                        @case('photo')
                            <div class="wq-upload-area" id="upload-{{ $question->id }}">
                                ...
                                <div style="display:flex;gap:8px;flex-wrap:wrap">
                                    <button type="button" class="wq-upload-btn" onclick="document.getElementById('cam-{{ $question->id }}').click()">
                                        <span>📷</span>
                                        {{ $lang === 'ar' ? 'التقاط صورة' : 'Take Photo' }}
                                    </button>
                                    <button type="button" class="wq-upload-btn" style="background:var(--gray-50);color:var(--gray-700);border:1px solid var(--gray-200)" onclick="document.getElementById('gal-{{ $question->id }}').click()">
                                        <span>🖼️</span>
                                        {{ $lang === 'ar' ? 'من المعرض' : 'Gallery' }}
                                    </button>
                                </div>
                                <input type="file" id="cam-{{ $question->id }}"
                                    accept="image/*" capture="environment"
                                    style="display:none"
                                    onchange="addFiles(this, '{{ $question->id }}')">
                                <input type="file" id="gal-{{ $question->id }}"
